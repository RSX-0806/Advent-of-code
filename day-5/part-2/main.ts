    ...
    if (op) {
      ...
      if (move > 1) {
        ...
        for (let i = 0; i < move; i++) {
          ...
          crate ? arranger.push(crate) : undefined;
        }
        const order = arranger.reverse();
        order.forEach((o) => {
          stack[to].push(o);
        });
      } else {
        for (let i = 0; i < move; i++) {
          const crate = stack[from].pop();
          crate ? stack[to].push(crate) : undefined;
        }
      }
    }
  }
...
/*
 * If 3/2 crates to move keep previous order
 */
